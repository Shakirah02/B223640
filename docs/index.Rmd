...
## Load and clean demographic datasets - "HB_names", "population_data" and "age_data"
...
```
# Processing Datasets into Wide Format
...
### Steps:
1.- Calculate the prescription rates for SSRIs and SNRIs per capita for both exam and non-exam periods.
  - This accounts for the total population in each health board.

2. - Incorporate the percentage of individuals aged 18–25 from the demographic data for each health board.

3. - Compute the percentage change in prescription rates between exam and non-exam periods to highlight   differences.

4. - Use the `gt` package to create a clean and visually appealing table for presentation.
...
### Seasonal Prescription Differences
...
The following code combines the exam and non-exam datasets (`examszndata` and `nonexamszndata`) into a single dataset, `joinedfulldata`. Key steps include:
- Cleaning column names for consistency.
- Joining with demographic datasets (`HB_names` and `population_data`) to enrich the data with health board names and population details.

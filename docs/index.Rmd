...
ggplot(summary_totals, aes(x = factor(year), y = total_paid_quantity, fill = factor(year))) +
  geom_bar(stat = "identity", position = "dodge", alpha = 0.7) +
  geom_line(aes(group = 1), color = "black", size = 1) +
  geom_point(aes(group = 1), color = "black", size = 2) +
  geom_text(aes(label = scales::comma(total_paid_quantity)), vjust = -0.5, size = 3.5) +
  facet_wrap(~ period) +
  labs(
